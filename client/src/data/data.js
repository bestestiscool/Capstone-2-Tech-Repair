const Data = {
  Header: {
    title: "Welcome to TechRepair Experts!",
    paragraph:
      "Your trusted partner for professional device repairs and cost estimation. From screen repairs to software recovery, we provide reliable solutions to keep your tech running smoothly and efficiently.",
  },
  About: {
    paragraph:
      "At TechRepair Experts, we're dedicated to giving you the best experience and service for all your repair needs. Our team of skilled professionals is passionate about what we do, and we take pride in being meticulous with every repair to ensure you get reliable, high-quality results every time.",
    Why: [
      "Screen Repairs for smartphones, tablets, and laptops, ensuring your display looks as good as new.",
      "Battery Replacements to keep your device powered up and performing at its best.",
      "Water Damage Recovery for those unexpected accidents.",
      "Charging Port and Connector Repairs so you can charge and connect with ease.",
    ],
    Why2: [
      "Hardware Upgrades, including RAM and Storage Enhancements for improved performance.",
      "Camera Repairs to restore your photo quality and video clarity.",
      "Software and Data Recovery Services to safeguard your important files and data.",
      "Reliability and quality service with a team that genuinely cares about your satisfaction.",
    ],
  },
  Gallery: [
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/01-large.jpg",
      smallImage: "img/portfolio/gallery/small/01-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/02-large.jpg",
      smallImage: "img/portfolio/gallery/small/02-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/03-large.jpg",
      smallImage: "img/portfolio/gallery/small/03-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/04-large.jpg",
      smallImage: "img/portfolio/gallery/small/04-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/05-large.jpg",
      smallImage: "img/portfolio/gallery/small/05-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/06-large.jpg",
      smallImage: "img/portfolio/gallery/small/06-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/07-large.jpg",
      smallImage: "img/portfolio/gallery/small/07-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/08-large.jpg",
      smallImage: "img/portfolio/gallery/small/08-small.jpg",
    },
    {
      title: "Project Title",
      largeImage: "img/portfolio/gallery/large/09-large.jpg",
      smallImage: "img/portfolio/gallery/small/09-small.jpg",
    },
  ],

  Services: [
    {
      icon: "fa fa-mobile",
      name: "Screen Repair",
      text: "Expert repairs for cracked or damaged screens on smartphones, tablets, and laptops, restoring them to like-new condition.",
    },
    {
      icon: "fa fa-battery-quarter",
      name: "Battery Replacement",
      text: "Professional battery replacement services to keep your device powered up and running efficiently.",
    },
    {
      icon: "fa fa-tint",
      name: "Water Damage Recovery",
      text: "Specialized recovery services for water-damaged devices to prevent further issues and restore functionality.",
    },
    {
      icon: "fa fa-plug",
      name: "Charging Port & Connector Repair",
      text: "Repair and replacement of charging ports and connectors for seamless charging and connectivity.",
    },
    {
      icon: "fa fa-cogs",
      name: "Button Repair",
      text: "Efficient repairs for malfunctioning buttons, including home, volume, and power buttons, ensuring smooth device operation.",
    },
    {
      icon: "fa fa-camera",
      name: "Camera Repair",
      text: "High-quality repairs for cameras, restoring photo and video quality to keep your memories sharp.",
    },
    {
      icon: "fa fa-hard-drive",
      name: "Hardware Upgrades",
      text: "Upgrades for RAM and storage to enhance device performance and increase capacity.",
    },
    {
      icon: "fa fa-database",
      name: "Software & Data Recovery",
      text: "Comprehensive software repairs and data recovery services to protect and restore your important files and data.",
    },
  ],

  Contact: {
    address: "4321 California St, San Francisco, CA 12345 ",
    phone: "+1 123 456 1234",
    email: "info@company.com",
    facebook: "fb.com",
    twitter: "twitter.com",
    youtube: "youtube.com",
  },
  Testimonials: [
    {
      img: "img/testimonials/01.jpg",
      text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sed dapibus leo nec ornare diam sedasd commodo nibh ante facilisis bibendum dolor feugiat at."',
      name: "John Doe",
    },
    {
      img: "img/testimonials/02.jpg",
      text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sed dapibus leo nec ornare diam sedasd commodo nibh ante facilisis bibendum dolor feugiat at."',
      name: "Johnathan Doe",
    },
    {
      img: "img/testimonials/03.jpg",
      text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sed dapibus leo nec ornare diam sedasd commodo nibh ante facilisis bibendum dolor feugiat at."',
      name: "John Doe",
    },
    {
      img: "img/testimonials/04.jpg",
      text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sed dapibus leo nec ornare diam sedasd commodo nibh ante facilisis bibendum dolor feugiat at."',
      name: "Johnathan Doe",
    },
    {
      img: "img/testimonials/05.jpg",
      text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sed dapibus leo nec ornare diam sedasd commodo nibh ante facilisis bibendum dolor feugiat at."',
      name: "John Doe",
    },
    {
      img: "img/testimonials/06.jpg",
      text: '"Lorem ipsum dolor sit amet, consectetur adipiscing elit. Duis sed dapibus leo nec ornare diam sedasd commodo nibh ante facilisis bibendum dolor feugiat at."',
      name: "Johnathan Doe",
    },
  ],
  Team: [
    {
      img: "img/team/01.jpg",
      name: "John Doe",
      job: "Director",
    },
    {
      img: "img/team/02.jpg",
      name: "Mike Doe",
      job: "Senior Designer",
    },
    {
      img: "img/team/03.jpg",
      name: "Jane Doe",
      job: "Senior Designer",
    },
    {
      img: "img/team/04.jpg",
      name: "Karen Doe",
      job: "Project Manager",
    },
  ],
  Features: [
    {
      icon: "fa fa-comments-o",
      title: "Lorem ipsum",
      text: "Lorem ipsum dolor sit amet placerat facilisis felis mi in tempus eleifend pellentesque natoque etiam.",
    },
    {
      icon: "fa fa-bullhorn",
      title: "Lorem ipsum",
      text: "Lorem ipsum dolor sit amet placerat facilisis felis mi in tempus eleifend pellentesque natoque etiam.",
    },
    {
      icon: "fa fa-group",
      title: "Lorem ipsum",
      text: "Lorem ipsum dolor sit amet placerat facilisis felis mi in tempus eleifend pellentesque natoque etiam.",
    },
    {
      icon: "fa fa-magic",
      title: "Lorem ipsum",
      text: "Lorem ipsum dolor sit amet placerat facilisis felis mi in tempus eleifend pellentesque natoque etiam.",
    },
  ],
};

export default Data;